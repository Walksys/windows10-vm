...
# Startup script (100% English & Dynamic Hardware Scaling)
RUN echo '#!/bin/bash\n\
set -e\n\
\n\
...
if [ -e /dev/kvm ]; then\n\
  ...
  KVM_ARG=""\n\
  CPU_ARG="qemu64"\n\
  VM_RAM="${RAM:-2048}"        # Default to 2GB on emulation\n\
  VM_CORES="${CORES:-1}"       # Default to 1 Core on emulation\n\
fi\n\
\n\
...
\n\
echo "========================================================================="\n\
echo "⚙️ Booting Windows 10 VM Profile:"\n\
echo "   -> Allocated RAM   : ${VM_RAM}MB / GB"\n\
echo "   -> Allocated Cores : ${VM_CORES} CPU(s)"\n\
echo "   -> Virtual Disk    : ${VM_DISK_SIZE}"\n\
echo "========================================================================="\n\
\n\
# Start QEMU with Windows-optimized dynamically injected settings\n\
qemu-system-x86_64 \\\n\
  $KVM_ARG \\\n\
